...
> Manage prepare-to-ship execution for [feature] using the current packet and asset/data inventory. Do not equate implementation complete, intent approved, or Chrome shipment with broad readiness. Load `modules/launch-execution.md`, `modules/standards-and-incubation-analysis.md`, `modules/implementation-and-issue-tracker-research.md`, `modules/developer-signals.md`, `modules/launch-acceptance.md`, and `modules/completion-loop.md`; this is completion-driven execution with machine-validated facts, saved browser evidence, comprehensive copy-paste examples, and verified friction closure—not planning or self-attestation.
>
> Derive a coverage manifest from the spec/IDL and implementation. Create independently runnable feature-detection/fallback and minimal behavior samples, focused option/failure/integration samples, and at least one realistic use-case demo. Launch the intended Chrome build and run them against the real implementation using `chrome-devtools-mcp`; exercise visible controls, inspect console/network/runtime state, and report exact pass/fail/blocked counts. If Chrome/MCP/platform access is unavailable, mark runtime blocked—do not substitute content or static checks. Build friction only from observed or reproduced evidence; keep reported-unverified issues and hypotheses separate.
>
